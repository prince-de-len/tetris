...
void TField::Burning() {
	...
	for (int coordinate_field_width = 0; coordinate_field_width < field_width; ++coordinate_field_width) { // проходим по всему полю снизу вверх
		counter = 0;

		static bool fillColumn;
		fillColumn = true;

		//for (int coordinate_field_height = field_height - 1; coordinate_field_height >= field_height - 10; --coordinate_field_height) { // ищем заполненный столбец
		//   
		//    if (field[coordinate_field_height][coordinate_field_width] != c_figure_down) {
		//        fillColumn = false;
		...
		//}
		for (int coordinate_field_height = field_height - 1; coordinate_field_height >= 0; --coordinate_field_height) { // ищем заполненный столбец
		...
		if (fillColumn) {
			...
			//for (int position_figure_x = coordinate_field_width; position_figure_x <= coordinate_field_width + 1; ++coordinate_field_width) {

//                memcpy(field[position_figure_x], field[position_figure_x - 1], sizeof(field[position_figure_x]));
//            }

			for (int coordinate_field_height = 0; coordinate_field_height <= field_height - 1; ++coordinate_field_height) { // ищем заполненный столбец
				if (counter == 10) {
					break;
				}
				if (field[coordinate_field_height][coordinate_field_width] == c_figure_down) {
					field[coordinate_field_height][coordinate_field_width] = c_field;  // заполняет игровое поле пустыми клетками
					++counter;
				}
